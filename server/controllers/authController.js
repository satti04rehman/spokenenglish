...
const login = async (req, res) => {
  ...
  try {
    ...
    if (!studentId || !password) {
      await logActivity(null, 'login_failed', null, 'Missing credentials', ip, false, 'No studentId or password provided');
    ...
    if (!user) {
      await logActivity(null, 'login_failed', null, `Invalid user: ${studentId}`, ip, false, 'User not found');
      return res.status(401).json({ message: 'Invalid credentials.' });
    }

    if (!user.isActive) {
      await logActivity(user._id, 'login_failed', null, 'Account inactive', ip, false, 'User account deactivated');
    ...
    if (user.lockUntil && user.lockUntil instanceof Date && user.lockUntil > new Date()) {
      ...
      await logActivity(user._id, 'login_failed', null, `Account locked (${secondsLeft}s remaining)`, ip, false, 'Account temporarily locked');
    ...
    if (!isMatch) {
      ...
      if (next >= MAX_FAILED_LOGINS) {
        ...
        await logActivity(user._id, 'login_failed', null, `Account locked after ${MAX_FAILED_LOGINS} failed attempts`, ip, false, 'Max failed login attempts exceeded');
      } else {
        await logActivity(user._id, 'login_failed', null, `Failed attempt #${next}`, ip, false, 'Invalid password');
      }

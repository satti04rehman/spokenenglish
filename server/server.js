  ...
  try {
    ...
    if (!existingAdmin) {
      ...
      await User.create({
        ...
        isActive: true,
        mustChangePassword: true, // SECURITY: Force admin to change password on first login
      });
      console.log('✅ Admin account seeded');
      console.log('   Login ID: admin');
      console.log('   Password: Check ADMIN_PASSWORD env variable');
      if (process.env.NODE_ENV === 'development') {
        console.warn('   ⚠️ Running in development. For production, set ADMIN_PASSWORD env variable.');
      }
    } else {
...
app.use(cors({
  origin: (origin, callback) => {
    ...
    // Parse RAILWAY_URL if available (Railway sets this automatically)
    ...
    // If the origin matches any of the allowed origins

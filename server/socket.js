...
const setupSocket = (server) => {
  // We use CORS aligned with Express for consistency
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('student_send_message', (data) => {
      ...
      io.to(`${socket.user.classId}_teachers`).emit('receive_message', payload);
    ...
    socket.on('teacher_send_message', (data) => {
      ...
      if (data.targetStudentId && data.targetStudentId !== 'all') {
        ...
        io.to(`${socket.user.classId}_student_${data.targetStudentId}`).emit('receive_message', payload);
        // Also ensure all teachers see the outbound Direct Message
        io.to(`${socket.user.classId}_teachers`).emit('receive_message', payload);
      } else {
        // Broadcast to entire class (everyone)
        io.to(socket.user.classId).emit('receive_message', payload);
